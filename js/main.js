...
const App = (() => {
  ...
  function initNav() {
    ...
    // Menú móvil (se inyecta en todas las páginas desde aquí)
    const inner = nav.querySelector(".nav__inner");
    if (!inner || nav.querySelector(".nav__burger")) return;

    const isHome = document.body.dataset.page === "home";
    const base = isHome ? "" : "index.html";
    const LINKS = [
      ["Trabajo", base + "#trabajo"],
      ["Enfoque", base + "#enfoque"],
      ["Proceso", base + "#proceso"],
      ["Curriculum", base + "#perfil"],
      ["Impacto de <em>Negocio</em>", "impacto.html"],
    ...
    menu.innerHTML =
      LINKS.map(([label, href], i) =>
        `<a class="mobile-menu__link" href="${href}" style="transition-delay:${60 + i * 55}ms">${label}</a>`
      ).join("") +
      `<p class="mobile-menu__meta">Santo Domingo, República Dominicana<br>Disponible para proyectos</p>`;
  ...
  function renderWorkIndex() {
    ...
    const rows = DATA.categorias.map((cat, i) => {
      ...
      const first = projs[0];
      // Rotación curada por categoría; fallback a la portada del primer proyecto
      let previews = [];
      if (cat.previews && first) {
        previews = cat.previews.map((n) => imgPath(first, n));
      } else if (first) {
        previews = [cover(first)];
      }
      ...
      return `
        <a class="work-row" href="categoria.html?cat=${cat.slug}" data-previews="${previews.join("|")}">
    ...
    // Preview flotante que sigue el cursor, con crossfade entre piezas
    if (!FINE_POINTER || REDUCED) return;
    ...
    let rot = null, srcs = [], idx = 0, front = imgA, back = imgB;
    (function loop() {
      px += (tx - px) * 0.12;
      py += (ty - py) * 0.12;
    ...
    const stopRot = () => { if (rot) { clearInterval(rot); rot = null; } };
    const crossfade = () => {
    ...
    wrap.querySelectorAll(".work-row").forEach((row) => {
      row.addEventListener("mouseenter", () => {
        srcs = (row.dataset.previews || "").split("|").filter(Boolean);
        ...
        stopRot();
        if (srcs.length > 1) rot = setInterval(crossfade, 1200);
      });
